...
use game_entity_component::gamejam::game::game_host::{get_game_data_kv_int, insert_components, play_animation, remove_component, set_game_data_kv_int, Collider, Direction, EventData, InsertableComponents};
...
impl DoorScript {
    fn new(settings: StartupSettings) -> Self {
        ...
        if game_state == 0 {
            ...
            play_animation("door_1", "closed", 1000, Direction::East, true);
        } else {
            insert_components(&[InsertableComponents::Attackable]);
            play_animation("door_1", "open", 1000, Direction::East, true);
        }
        Self {
...
impl GuestGameEntity for DoorScript {
    ...
    fn animation_finished(&self, animation_name: String) -> () {
        ...
        play_animation("door_1", next_anim_name, 1000, Direction::East, true);
    }
        ...
        match evt.data {
            EventData::Trigger(id) => {
                if id == self.trigger_value && self.state.get() == 0 {
                    ...
                    play_animation("door_1", "opening", 1000, Direction::East, false);
                }
            }
        }
    }
